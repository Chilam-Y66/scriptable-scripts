...
function safeGetKeychain(key){ try{return Keychain.contains(key)?Keychain.get(key):null}catch(e){return null} }
function safeSetKeychain(key,val){ try{Keychain.set(key,val);return true}catch(e){return false} }
function safeRemoveKeychain(key){ try{Keychain.remove(key)}catch(e){} }
...
function getConfiguredIds(){ try{var s=Keychain.get("api_bal_viewer_config_ids");return s?JSON.parse(s):[]}catch(e){return[]} }
function saveConfiguredIds(ids){ try{Keychain.set("api_bal_viewer_config_ids",JSON.stringify(ids))}catch(e){} }
function getThresholds(){ try{var s=Keychain.get("api_bal_viewer_thresholds");return s?JSON.parse(s):{}}catch(e){return{}} }
function saveThresholds(th){ try{Keychain.set("api_bal_viewer_thresholds",JSON.stringify(th))}catch(e){} }
...
async function fetchBalance(provider, apiKey){
  ...
  try{
    ...
    if(provider.id==="openai"){
      ...
      req.headers={"Authorization":"Bearer "+apiKey};
      var json=await req.loadJSON();
      if(json&&json.hard_limit_usd!==undefined) return{success:true, balance:json.hard_limit_usd, raw:json};
      if(json&&json.error) return{success:false, error:"API错误:"+JSON.stringify(json.error)};
      return{success:false, error:"返回格式异常:"+JSON.stringify(json).substring(0,200)};
    }
    if(provider.id==="kimi"){
      var req=new Request("https://api.moonshot.cn/v1/users/me/balance");
      req.headers={"Authorization":"Bearer "+apiKey};
      var json=await req.loadJSON();
      if(json&&json.balance!==undefined) return{success:true, balance:json.balance, raw:json};
      if(json&&json.error) return{success:false, error:"API错误:"+JSON.stringify(json.error)};
      return{success:false, error:"返回格式异常:"+JSON.stringify(json).substring(0,200)};
    }
    return{success:true, balance:null, notice:"请在对应控制台查看余额"};
  }catch(e){ return{success:false, error:String(e).substring(0,300)}; }
}
...
async function fetchAll(){
  ...
  for(var i=0;i<ids.length;i++){
    var id=ids[i];
    var prov=null;
    for(var j=0;j<PROVIDERS.length;j++){ if(PROVIDERS[j].id===id){prov=PROVIDERS[j];break;} }
    if(!prov) continue;
    var apiKey=safeGetKeychain("api_bal_viewer_"+id);
    if(!apiKey){ results.push({id:id,provider:prov,success:false,error:"未配置Key"}); continue; }
    var balR=await fetchBalance(prov,apiKey);
    var usageR=await fetchUsage(prov,apiKey);
    results.push({id:prov.id,provider:prov,success:balR.success,balance:balR.balance,unit:prov.unit,error:balR.error||null,todayTokens:usageR.todayTokens,monthTokens:usageR.monthTokens,todayDetail:usageR.todayDetail,monthDetail:usageR.monthDetail});
  }
  return results;
}

function checkThresholds(results){
  var th=getThresholds(); var alerts=[];
  for(var i=0;i<results.length;i++){
    ...
    if(balNum<=threshold) alerts.push(r.provider.name+" 余额低: "+fmtNum(balNum)+" "+r.unit);
  }
  return alerts;
}
...
async function addProvider(){
  var avail=[];
  var ids=getConfiguredIds();
  for(var i=0;i<PROVIDERS.length;i++){ if(ids.indexOf(PROVIDERS[i].id)<0) avail.push(PROVIDERS[i]); }
  if(avail.length===0){
    var nAlert=new Alert();
    nAlert.title="提示"; nAlert.message="所有服务商已配置";
    nAlert.addAction("确定");
    nAlert.addCancelAction("");
  ...
  while(true){
    ...
    kAlert.addAction("保存");
    kAlert.addAction("清除Key重新输入");
    kAlert.addCancelAction("取消");
    var kIdx=await kAlert.presentAlert();
    if(kIdx===2) return; // 取消
    if(kIdx===1){ continue; } // 清除重新输入
    var key=kAlert.textFieldValue(0);
    if(!key||key.trim()===""){
      var eAlert=new Alert();
      eAlert.title="错误"; eAlert.message="API Key 不能为空";
      eAlert.addAction("确定");
      eAlert.addCancelAction("");
    ...
    okAlert.title="成功";
    okAlert.message=prov.name+" 已添加\nKey前10位:"+key.trim().substring(0,10)+"...";
    okAlert.addAction("继续添加");
    okAlert.addCancelAction("返回");
    var okIdx=await okAlert.presentAlert();
    if(okIdx===0) continue; // 继续添加
    break; // 返回
  }
}
...
async function editProvider(){
  ...
  if(dIdx===0){
    ...
    if(kIdx===0){
      ...
      if(newKey&&newKey.trim()!==""){ safeSetKeychain("api_bal_viewer_"+pid,newKey.trim()); }
    }
  }
  if(dIdx===1){
...
async function showThresholdMenu(){
  while(true){
    ...
    if(idx===0){
      ...
      if(eIdx===0){ var v=Number(eAlert.textFieldValue(0)); if(v===v&&v>0){th["_global"]=v;saveThresholds(th);} }
    }else{
      var pid=ids[idx-1];
      var p=null;
      ...
      eAlert.addCancelAction("取消");
      var eIdx=await eAlert.presentAlert();
      if(eIdx===0){ var v=eAlert.textFieldValue(0); if(v&&v.trim()!==""){th[pid]=Number(v);}else{delete th[pid];} saveThresholds(th); }
  ...
  (async function(){
    ...
    try{
      ...
      if(results.length===0){
        ...
        t.font=Font.systemFont(11); t.textColor=Color.gray();
      }else{
        for(var i=0;i<results.length;i++){
          var r=results[i];
          var line=r.provider.icon+" ";
          if(r.success&&r.balance!==null){ line+=fmtNum(r.balance)+" "+r.unit; }else{ line+="error"; }
      ...
      t.font=Font.systemFont(11); t.textColor=Color.red();
    }
    widget.addSpacer();
    var footer=widget.addText(todayStr());
    footer.font=Font.systemFont(9); footer.textColor=Color.gray();
...
if(args.widgetParameter==="silent"){
  (async function(){
    try{
      ...
      if(alerts.length>0) Notification.schedule("余额预警",alerts.join("\n"));
    }catch(e){}
    Script.complete();
  })();
}
...
// ========== 正常启动 ==========
if(!config.runsInWidget&&args.widgetParameter!=="silent"){ mainMenu(); }
